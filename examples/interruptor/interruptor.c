...
homekit_characteristic_t name = HOMEKIT_CHARACTERISTIC_(NAME, "Interruptor");
...
homekit_accessory_t *accessories[] = {
    HOMEKIT_ACCESSORY(.id=1, .category=homekit_accessory_category_outlet, .services=(homekit_service_t*[]){
        HOMEKIT_SERVICE(ACCESSORY_INFORMATION, .characteristics=(homekit_characteristic_t*[]){
            ...
            HOMEKIT_CHARACTERISTIC(MODEL, "Interruptor"),
        ...
        HOMEKIT_SERVICE(OUTLET, .primary=true, .characteristics=(homekit_characteristic_t*[]){
            HOMEKIT_CHARACTERISTIC(NAME, "Interruptor"),
            &switch_on,
...
void create_accessory_name() {
    ...
    int name_len = snprintf(NULL, 0, "Interruptor-%02X%02X%02X",
                            macaddr[3], macaddr[4], macaddr[5]);
    char *name_value = malloc(name_len+1);
    snprintf(name_value, name_len+1, "Interruptor-%02X%02X%02X",
             macaddr[3], macaddr[4], macaddr[5]);
...
void user_init(void) {
    ...
    wifi_config_init("Interruptor", NULL, on_wifi_ready);
    gpio_init();
